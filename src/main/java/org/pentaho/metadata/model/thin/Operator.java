...
public enum Operator implements Serializable {
   ...
   */
  /*
   * public static Operator[] values(boolean stringType){ Operator[] vals = Operator.values(); List<Operator> ops = new
   * ArrayList<Operator>(); for(int i=0; i < vals.length; i++){ if (vals[i].operatorType == 2) { ops.add(vals[i]); }
   * else if(vals[i].operatorType == 0 && stringType){ ops.add(vals[i]); } else if (vals[i].operatorType == 1 &&
      ...
      case EXACTLY_MATCHES:
        if ( value.length == 1 ) {
          ...
          sb.append( "IN(" ) //$NON-NLS-1$
              .append( columnName ).append( "; " ); //$NON-NLS-1$
          for ( int idx = 0; idx < value.length; idx++ ) {
            if ( idx > 0 ) {
              sb.append( ";" ); //$NON-NLS-1$
            }
