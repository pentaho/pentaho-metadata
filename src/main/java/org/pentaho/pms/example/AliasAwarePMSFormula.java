...
public class AliasAwarePMSFormula extends PMSFormula {
  ...
  private Map<String, AdvancedMQLQuery.AliasedSelection> aliasedSelectionMap =
      new HashMap<String, AdvancedMQLQuery.AliasedSelection>();
   ...
   */
  public AliasAwarePMSFormula( BusinessModel model, DatabaseMeta databaseMeta, String formulaString,
   ...
   *           throws an exception if we're missing anything important
   */
  public AliasAwarePMSFormula( BusinessModel model, BusinessTable table, DatabaseMeta databaseMeta,
   ...
   *           throws an exception if we're missing anything important
   */
  public AliasAwarePMSFormula( BusinessModel model, BusinessTable table, String formulaString, String aliasName )
    throws PentahoMetadataException {
      ...
      for ( Selection selection : selections ) {
        ...
        if ( aliasedSelection.getAlias() != null && aliasedSelection.getAlias().equals( names[0] ) ) {
          ...
          if ( column != null ) {
            ...
            AdvancedMQLQuery.AliasedSelection sel =
                new AdvancedMQLQuery.AliasedSelection( column, aliasedSelection.getAlias() );
  ...
  protected void renderContextLookup( StringBuffer sb, String contextName, String locale ) {
    ...
    if ( sel != null ) {
      ...
      AdvancedSQLGenerator.SQLAndAliasedTables sqlAndTables =
          AdvancedSQLGenerator.getSelectionSQL( getBusinessModel(), sel, getDatabaseMeta(), locale );
    ...
    if ( column == null ) {
      ...
      sb.append( " " ); //$NON-NLS-1$
      AdvancedSQLGenerator.SQLAndAliasedTables sqlAndTables =
          AdvancedSQLGenerator.getSelectionSQL( getBusinessModel(), selection, getDatabaseMeta(), locale );
      sb.append( sqlAndTables.getSql() );
      sb.append( " " ); //$NON-NLS-1$
  ...
  List<AdvancedSQLGenerator.AliasedPathBusinessTable> aliasedTables =
      new ArrayList<AdvancedSQLGenerator.AliasedPathBusinessTable>();
