...
public class RowLevelSecurity implements Cloneable {
  ...
  public Object clone() throws CloneNotSupportedException {
    return new RowLevelSecurity( type, globalConstraint, roleBasedConstraintMap != null
        ? cloneRoleBasedConstraintMap( roleBasedConstraintMap ) : null );
  }
  ...
  public String toXML() {
    ...
    xml.append( START_ELEM_NAME_BEGIN ).append( ELEM_ROW_LEVEL_SECURITY ).append( SPACE ).append( ATTR_TYPE ).append(
        EQUALS );
    if ( isGlobal() ) {
  ...
  /**
   * WG: Note, this method is not used at the moment, we may in the future use this if MQL is partially executed before
   * passing down the SQL to database engines.
   */
  ...
  public static void main( String[] args ) {
    String formula =
        "OR(AND(IN(\"suzy\";USER());[BT_CUSTOMER_W_TER_CUSTOMER_W_TER.BC_CUSTOMER_W_TER_COUNTRY] = \"USA\"))";
    // "expand" USER() function (regex: escape parentheses and escape backslash that escapes parentheses
