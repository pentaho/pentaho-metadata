...
public class MQLQueryImplIT extends MetadataTestBase {
  ...
  public void testCircularJoin() throws Exception {
    ...
    for ( int i = 0; i < path.size(); i++ ) {
      ...
      assertTrue( rel.equals( rl1 ) || rel.equals( rl3 ) || rel.equals( rl6 ) || rel.equals( rl5 ) );
    }
  ...
  public void testLimitedQuery() throws Exception {
    ...
    assertEqualsIgnoreWhitespaces( "SELECT TOP 10 DISTINCT \n" //$NON-NLS-1$
        + "          bt1.pc1 AS COL0\n" //$NON-NLS-1$
        + "FROM \n" //$NON-NLS-1$
        + "          (select * from mytable) bt1\n", //$NON-NLS-1,
      query.getQuery() ); //$NON-NLS-1$
