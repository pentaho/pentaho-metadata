...
{
    public static final int MIDDLE = Props.getInstance().getMiddlePct();
    ...
    {
        return getControls(parentComposite, utilityInterface, message, null, null, concept, conceptPropertyInterfaces, locales, securityReference);
    }
            ...
            )
    {
        ...
        if (defaultProperties!=null && subject!=null)
        {
            ...
            for (int i=0;i<list.size();i++)
            {
                ...
                if (property==null)
                {
                    ...
                    //
                    concept.addProperty(DefaultPropertyID.getDefaultEmptyProperty(defaultProperty.getConceptPropertyType(), defaultProperty.getName()));
                    property = concept.getProperty(defaultProperty.getName()); // it's empty
                }
                ...
                FormData fdLine = new FormData();
                fdLine.left   = new FormAttachment(5, 0);
                fdLine.right  = new FormAttachment(100, -MARGIN);
                ...
                */
                ...
                ConceptPropertyWidgetInterface widgetInterface = (ConceptPropertyWidgetInterface) conceptPropertyInterfaces.get(defaultProperty.getName());
        ...
        else
        {
            ...
            for (int i=0;i<ids.length;i++)
            {
                ...
                */
                ...
                // In case it's a default property we can look up a real description
                //
                ...
                // Set to changed if we did add a default concept...
                ConceptPropertyWidgetInterface widgetInterface = (ConceptPropertyWidgetInterface) conceptPropertyInterfaces.get(ids[i]);
                if (widgetInterface!=null)
                {
                    widgetInterface.setChanged(changed);
                }
        ...
        // Not just the parent composite, re-consider the layout of the whole shell
    ...
    private static final void addProperty (
               final Composite parentComposite, 
               final ConceptUtilityInterface utilityInterface, 
               final String message, 
               final Class subject, 
    ...
    private static final void delProperty (
                final Composite parentComposite, 
                final ConceptUtilityInterface utilityInterface, 
                final String message, 
                final Class subject, 
    ...
    public static final Control addControl(Composite composite, ConceptInterface concept, String description, ConceptPropertyInterface property, Control previousControl, Map conceptPropertyInterfaces, Locales locales, SecurityReference securityReference)
    {
        Props props = Props.getInstance();
        ...
        switch(type.getType())
        {
            case ConceptPropertyType.PROPERTY_TYPE_STRING           : lastControl = ConceptPropertyStringWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_DATE             : lastControl = ConceptPropertyDateWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_NUMBER           : lastControl = ConceptPropertyNumberWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_COLOR            : lastControl = ConceptPropertyColorWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_FONT             : lastControl = ConceptPropertyFontWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_FIELDTYPE        : lastControl = ConceptPropertyFieldTypeWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_AGGREGATION      : lastControl = ConceptPropertyAggregationWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_BOOLEAN          : lastControl = ConceptPropertyBooleanWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_DATATYPE         : lastControl = ConceptPropertyDataTypeWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_LOCALIZED_STRING : lastControl = ConceptPropertyLocalizedStringWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces, locales); break;
            case ConceptPropertyType.PROPERTY_TYPE_TABLETYPE        : lastControl = ConceptPropertyTableTypeWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_URL              : lastControl = ConceptPropertyURLWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_SECURITY         : lastControl = ConceptPropertySecurityWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces, securityReference); break;
            case ConceptPropertyType.PROPERTY_TYPE_ALIGNMENT        : lastControl = ConceptPropertyAlignmentWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            case ConceptPropertyType.PROPERTY_TYPE_COLUMN_WIDTH     : lastControl = ConceptPropertyColumnWidthWidget.getControl(composite, concept, id, previousControl, conceptPropertyInterfaces); break;
            default: break;
        }
        ...
        if (parent!=null || inherit!=null || security!=null)
        {
            ...
            final ConceptPropertyWidgetInterface dialogInterface = (ConceptPropertyWidgetInterface) conceptPropertyInterfaces.get(id);
            if (dialogInterface!=null)
            {
                final SelectionAdapter selectionAdapter = new SelectionAdapter() 
                 ...
                 // if there is no parent property for this, the overwrite is always selected and the checkbox is disabled.
                 ...
                 {
                     // If the parent property exists and the child property too: enable overwrite
    ...
    public static final boolean setPropertyValues(Shell shell, Map conceptWidgetInterfaces)
    {
        ...
        for (Iterator iter = keySet.iterator(); iter.hasNext() ; )
        {
            ...
            ConceptPropertyWidgetInterface widgetInterface = (ConceptPropertyWidgetInterface) conceptWidgetInterfaces.get(name);
            if (widgetInterface!=null)
            {
                ...
                try
                {
                    ...
                    {
                        // If we had a parent or inherited concept & the overwrite goes away: clear the value...
                        if ((concept.hasParentConcept() || concept.hasInheritedConcept() || concept.hasSecurityParentConcept()) && !widgetInterface.isOverwrite())
                        {
                            
                            // clear the child property if there is one and if there is also one defined above this concept level.
                            //
                            ...
                            if (property!=null && (parentProperty!=null || inheritedProperty!=null || securityProperty!=null))
                            {
