...
public class BusinessView extends ConceptUtilityBase implements ChangedFlagInterface, Cloneable, ConceptUtilityInterface, UndoInterface
{
    ...
    public RelationshipMeta findRelationship(String from, String to)
    {
        for (int i = 0; i < nrRelationships(); i++)
        {
            ...
            if (relationshipMeta.getTableFrom().getId().equals(from) && relationshipMeta.getTableTo().getId().equals(to)) return relationshipMeta;
        }
        return null;
    }
    ...
    public RelationshipMeta findRelationship(String id)
    {
        ...
        {
            RelationshipMeta relationshipMeta = getRelationship(i);
            if (relationshipMeta.toString().equals(id)) return relationshipMeta;
        }
        return null;
    }
    ...
    public int nrSelected()
    {
        ...
        {
            if (getBusinessTable(i).isSelected()) count++;
        }
        return count;
    }
    ...
    public BusinessTable getSelected(int nr)
    {
        ...
        for (i = 0; i < nrBusinessTables(); i++)
        {
            ...
            {
                if (nr == count) return getBusinessTable(i);
                count++;
            }
        }
        return null;
    ...
    public String getSelectedName(String locale, int nr)
    {
        ...
        if (table != null) { return table.getConcept().getName(locale); }
        return null;
    }
    ...
    public BusinessTable getTable(int x, int y, int iconsize)
    {
        for (int i = nrBusinessTables() - 1; i >= 0; i--) // Back to front because drawing goes from start to end
        {
            ...
            if (table.isDrawn()) // Only consider tables that are on the canvas
            {
                ...
                if (p != null)
                {
                    if (x >= p.x && x <= p.x + iconsize && y >= p.y && y <= p.y + iconsize) { return table; }
                }
            }
        }
        return null;
    ...
    public RelationshipMeta findRelationshipUsing(BusinessTable one, BusinessTable two)
    {
        ...
        {
            RelationshipMeta relationship = getRelationship(i);
            if (relationship.isUsingTable(one) && relationship.isUsingTable(two)) { return getRelationship(i); }
        }
    ...
    public Vector getShortestPathsBetween(BusinessTable tabs[])
    {
        ...
        for (int i = 0; i < tabs.length - 1; i++)
        {
            ...
            if (min > 0) onlyKeepSize(vector, min);
    ...
    public Path getShortestPathBetween(BusinessTable tabs[])
    {
        ...
        for (int i = 0; i < paths.size(); i++)
        {
            ...
            if (path.size() < minScore || (path.size() == minScore && path.score() < minSize)) minPath = path;
        }
     ...
     */
    public String getSQL(BusinessColumn selectedColumns[], WhereCondition conditions[], String locale, boolean useDisplayNames)
    {
     ...
     * @return a SQL query based on a column selection, conditions and a locale
     */
    public String getSQL(BusinessColumn selectedColumns[], WhereCondition conditions[], OrderBy[] orderBy, String locale, boolean useDisplayNames)
    {
        String sql = null;
    ...
    public String getSQL(BusinessColumn selectedColumns[], Path path, WhereCondition conditions[], OrderBy[] orderBy, String locale,
    ...
    public int getMinimumSize(Vector paths)
    {
        ...
        for (int i = 0; i < paths.size(); i++)
        {
            ...
            if (p.size() > 0 && p.size() < min) min = p.size();
        }
    ...
    private synchronized void quickSort(Vector elements, int lowIndex, int highIndex)
    {
        ...
        { // loop from low to high to find a candidate for swapping
            ...
            if (lowToHighIndex == highToLowIndex) // one last element, may go in either part
            {
                ...
                if (lowToHighIndex < highToLowIndex) // not last element yet
                {
    ...
    public BusinessTable findBusinessTable(String locale, String tablename)
    {
        for (int i = 0; i < nrBusinessTables(); i++)
        {
            ...
            if (displayName != null && displayName.equals(tablename)) return businessTable;
        }
        for (int i = 0; i < nrBusinessTables(); i++)
        {
            BusinessTable businessTable = getBusinessTable(i);
            String id = businessTable.getId();
            if (id != null && id.equals(tablename)) return businessTable;
        }
        return null;
    }
    ...
    public BusinessTable findBusinessTable(String tableID)
    {
        for (int i = 0; i < nrBusinessTables(); i++)
        {
            BusinessTable businessTable = getBusinessTable(i);
            String id = businessTable.getId();
            if (id != null && id.equals(tableID)) return businessTable;
        }
        return null;
    }
    ...
    public BusinessTable findBusinessTable(BusinessColumn businessColumn)
    {
        ...
        {
            BusinessTable businessTable = getBusinessTable(i);
            if (businessTable.findBusinessColumn(businessColumn.getId()) != null) return businessTable;
        }
        return null;
    }
    ...
    public boolean hasChanged()
    {
        ...
        if (haveRelationshipsChanged()) return true;
        if (rootCategory.hasChanged()) return true;
        if (haveOlapDimensionsChanged()) return true;
        if (haveOlapCubesChanged()) return true;
    ...
    private boolean haveRelationshipsChanged()
    {
        for (int i = 0; i < nrRelationships(); i++)
        {
            if (getRelationship(i).hasChanged()) return true;
        }
        return false;
    }
    ...
    public boolean haveOlapDimensionsChanged()
    {
        for (int i = 0; i < olapDimensions.size(); i++)
        {
            ...
            if (olapDimension.hasChanged()) return true;
        }
        return false;
    }
    ...
    public boolean haveOlapCubesChanged()
    {
        for (int i = 0; i < olapCubes.size(); i++)
        {
            ...
            if (olapCube.hasChanged()) return true;
        }
        return false;
    }
    ...
    public OlapDimension findOlapDimension(String name)
    {
        for (int i = 0; i < olapDimensions.size(); i++)
        {
            ...
            if (dimension.getName().equalsIgnoreCase(name)) return dimension;
        }
        return null;
    }
    ...
    public OlapCube findOlapCube(String name)
    {
        for (int i = 0; i < olapCubes.size(); i++)
        {
            ...
            if (cube.getName().equalsIgnoreCase(name)) return cube;
        }
        return null;
    }
