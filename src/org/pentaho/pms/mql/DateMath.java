 ...
 *
 * Copyright (c) 2005 - 2009 Pentaho Corporation..  All rights reserved.
 */
...
public class DateMath {
   ...
   * 
   *           public static String claculateDateString(final String expressionWithFormat) { return
   *           DateMath.calculateDateString(null, expressionWithFormat, null); }
   * 
   *           /** Calculates a date, returning the formatted string version of the calculated date. The method is a
   *           short cut for {@link #calculateDate(Calendar,String,Locale)
   ...
   *           if <tt>expressionWithFormat</tt> is invalid.
   * 
   *           public static String calculateDateString(final Calendar date, final String expressionWithFormat) { return
   *           DateMath.calculateDateString(date, expressionWithFormat, null); }
   * 
   ...
   *           if <tt>expressionWithFormat</tt> is invalid.
   * 
   *           public static String calculateDateString(final Calendar date, final String expressionWithFormat, final
   *           Locale locale) { int index = expressionWithFormat.indexOf(';'); String expression; String pattern = null;
   *           Calendar target = (date == null) ? Calendar.getInstance() : date; DateFormat format; Locale myLocale;
   * 
   *           if (index >= 0) { pattern = expressionWithFormat.substring(index + 1); expression =
   *           expressionWithFormat.substring(0, index); } else { expression = expressionWithFormat; }
   * 
   *           target = DateMath.calculateDate(date, expression);
   * 
   *           myLocale = (locale == null) ? LocaleHelper.getLocale() : locale; if (myLocale == null) { myLocale =
   *           LocaleHelper.getDefaultLocale(); }
   * 
   *           if (pattern != null) { format = new SimpleDateFormat(pattern, myLocale); } else { format =
   *           DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, myLocale); }
   * 
