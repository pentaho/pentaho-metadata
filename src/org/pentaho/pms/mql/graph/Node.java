...
public class Node implements GraphElement, Comparable {
	...
	/**
	 * Returns true if this <code>Node</code> is known to be required 
	 ...
	 * functions to determine if this <code>Node</code> is currently in the queue waiting
	 * to be processed.
	 * 
	...
	/**
	 * Adds an <code>Arc</code> that should be required if this <code>Node</code>
	 * is required
	 * 
	 ...
	 * @throws ConsistencyException	When assignment is inconsistent with graph constraints
	 * 								or an attempt to set a different value when one
	 * 								is already set
	 */
	...
	/**
	 * Returns true if this node was successfully marked
	 * as not required
	 * 
		...
		// if only 1 arc to this node and it is not yet marked as required we can prune it 
			...
			// when multiple paths exist, make sure not more than one is possible
				...
				for (Arc arc: arcs) {
					...
					if (!arc.isNotRequired()) possiblePaths++;
	...
	/**
	 * Returns true if the target node can be reached from this node
	 * to a target.
	 * 
	...
	/**
	 * Returns true if the all the nodes in target list can be reached 
	 * from this node 
	 * 
	...
	/**
	 * Returns true if the target node can be reached from the current node
	 * through some other <code>Arc</code> than this one.
	 * 
	...
	public String toString() {
	  StringBuffer buff = new StringBuffer();
		...
		if ( (table != null) && (table.getPhysicalTable() != null)) {
		  buff.append(table.getPhysicalTable().getDisplayName(""));
		} else if (table != null) {
		  buff.append(table.getDisplayName("")); // Business
		} else {
