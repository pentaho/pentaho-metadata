...
/**
 * Class that build a Node-Arc graph based on <code>BusinesTable</code> and
 * <code>RelationshipMeta</code> objects specified in a <code>BusinessModel</code>.
 * It attempts to use Arc Consistency Optimization to find a <code>Path</code> that
 * utilizes the smallest number of relationships to include a list of required
 * tables.
 */
public class MqlGraph implements GraphElementChangeListener {
	...
	/**
	 * Calculates and returns a path that satisfies the required tables list
	...
	public Path getPath(PathType searchTechnique, List<BusinessTable> requiredTables) {
		...
		if (reset(requiredTables) && isValid(searchTechnique)) {
			...
			for (Arc arc: arcs) {
				if (arc.isRequired()) {
				  if ( logger.isDebugEnabled() ) logger.debug("Arc selected for path: " + arc);
					path.addRelationship(arc.getRelationship());
				}
				else
				  if ( logger.isDebugEnabled() ) logger.debug("Arc not used for path: Requirement Known[" + arc.isRequirementKnown() + "], Required[" + arc.isRequired() + "]");
			}
			
			if (logger.isDebugEnabled()) {
				for (Node n: nodes)
				  if ( logger.isDebugEnabled() ) logger.debug("Node selection state: Requirement Known[" + n.isRequirementKnown() + "], Required[" + n.isRequired() + "]");
			}
			if (path.size()>0)
				return path;
		}
	...
	/**
	 * Performs work necessary to bind all nodes that have not been assigned
	 * a requirement value yet
	 * 
	...
	private void search(PathType searchTechnique) throws ConsistencyException {
		...
		if ( logger.isDebugEnabled() ) logger.debug("initial solution found - Rating[" + bestKnown.getRating() + "]");
			...
			try {
				...
				while(lastSolution != null) {
				  logger.debug("continuing search for more solutions from last one located");
					...
					if ( logger.isDebugEnabled() ) logger.debug("new solution located - Rating[" + bestKnown.getRating() + "]");
					
					if (lastSolution != null) {
					  if (logger.isTraceEnabled()) logger.trace("New solution is better than previously best known, continuing from it");
	...
	/**
	 * Attempts to find next valid solution to the graph depending on what type of
	 * <code>PathType</code> is desired.
	 * 
	 * @param searchTechnique	Indicates type of search that should be performed
	 * @param prevSolution		Previous solution to allow this search to continue from that point
	 * @return rating for the resulting solution
	 * @throws ConsistencyException When determine that graph is impossible to satisfy
	 */
	private Solution searchForNextSolution(PathType searchTechnique, Solution prevSolution) throws ConsistencyException {
		...
		if (prevSolution!=null) {
			ListIterator<SearchDirection> pathIter = searchPath.listIterator(searchPath.size());
			...
			while (pathIter.hasPrevious()) {
				...
				if (attemptArcAssignment(arc, secondDirection)) {
					...
					// just be skipped
					try {
						checkCurrentStateForImprovement(searchTechnique, prevSolution.getRating());
						break;
					}
					catch(ConsistencyException cx) {}
				}
		...
		for (Arc a: arcs) {
			if (!a.isRequirementKnown()) {
				...
				else if (attemptArcAssignment(a, secondDirection))
					searchPath.add(secondDirection);
				...
				if (prevSolution != null)
					rating = checkCurrentStateForImprovement(searchTechnique, prevSolution.getRating());
			}
		}
		
		// compute rating if never computed
		if (rating < 0)
			rating = checkCurrentStateForImprovement(searchTechnique, Integer.MAX_VALUE);
		
		// return solution to graph problem
		Solution s = new Solution(arcs, rating, searchPath, searchArcs);
		return s;
	}
	...
	/**
	 * Checks that the current state of the graph is better than (or has the potential to be better)
	 * than a minimum required value.
	 * 
	 * @param searchTechnique	Technique being used in searching to determine rating method
	 * @param minRating			Minimum value that solution must be better than
	 * @throws ConsistencyException If current state of graph is not better than minimum rating
	 */
	private int checkCurrentStateForImprovement(PathType searchTechnique, int minRating) throws ConsistencyException {
		int rating = 0;
	...
	/**
	 * Attempts to assign an arc to a given requirement status
	 * and returns true if consistency may still be possible
	 */
	private boolean attemptArcAssignment(Arc arc, SearchDirection direction) {
		...
		try {
		  if ( logger.isDebugEnabled() )  logger.debug("Attempting move - Direction[" + direction + "], Arc[" + arc + "]");
	...
	/**
	 * Called to reset the search stack to original state before
	 * searching began
	 */
	...
	/**
	 * Performs work of propagating changes from source nodes
	 * to target nodes until consistency is reached
	 * 
	...
	/**
	 * Creates a new arc and records appropriate dependencies in internal
	...
	/**
	 * Class that holds a possible solution to the graph problem for use
	 * during searching
	 */
	private static class Solution {
		...
		Solution(List<Arc> arcs, int rating, List<SearchDirection> searchPath, List<Arc> searchArcs) {
			...
			for (Arc a: arcs) solutionValues.add(a.isRequired());
		}
