 ...
 * 
 */
public class ImpalaDialect extends DefaultSQLDialect {

  /**
   * Impala only supports the equals operator in join conditions.  This pattern is used to detect the existence of invalid
  ...
  public ImpalaDialect() {
    super("IMPALA"); //$NON-NLS-1$
  }

  /**
   * Determine if this dialect can be loaded in the current environment.
   *
   * @return True if the Impala Database Meta Plugin is registered with the Kettle environment
   */
  public static final boolean canLoad() {
    try {
      return DatabaseMeta.getDatabaseInterface("IMPALA") != null;
  ...
  protected void generateSelect(SQLQueryModel query, StringBuilder sql) {
    ...
    for (SQLSelection selection : query.getSelections()) {
      ...
      if(isDriverVersion(0,6)) {
        // Only Impala version 0.6 and beyond support column aliases
  ...
  private String getFromClauseWithTables(SQLQueryModel query) {
    ...
    while (iter.hasNext()) {
      // Impala does not support more than one table reference.  When more than one table is 
  ...
  /**
   * Checks if a formula is a valid Impala join condition.
   * 
   * @param formula SQL where (join) formula
   * @return True if the formula can be used in the ON condition of a join in Impala.
   */
  ...
  protected void generateGroupBy(SQLQueryModel query, StringBuilder sql) {
    if (query.getGroupBys().size() > 0) {
      ...
      for (SQLSelection groupby : query.getGroupBys()) {
        ...
        // Impala does not support column aliases
  ...
  protected void generateOrderBy(SQLQueryModel query, StringBuilder sql) {
    // Impala does not support column aliases and due to this bug: Due to https://issues.apache.org/jira/browse/HIVE-1449
    ...
    if (query.getOrderBys().size() > 0) {
      ...
      for (SQLOrderBy orderby : query.getOrderBys()) {
        ...
        // Impala does not support column aliases or table qualifiers used in ORDER BY.
  ...
  protected synchronized void initDriverInfo() {
    ...
    try {
      ...
      Class<?> driverClass = Class.forName("org.apache.hive.jdbc.HiveDriver"); //$NON-NLS-1$
      if(driverClass != null) {
