...
public interface ModelProvider {
	...
	/**
	 * Returns an array of model descriptions. If the provider id does not match the current
	 * provider returns an empty array.
	 * @param provider
	 * @param group Optional the group id of the models to search
	 * @param match Optional a string to match on the model name/description
	 * @return
	 */
	public ModelInfo[] getModelList(String provider, String group, String match);

	/**
	 * Exexcutes a query and returns the results in a DataTable object. If row limit is specified
	 * returns only the specified number of rows or less. rowLimit of -1 indicates all rows.
	 * @param query
	 * @param rowLimit
	 * @return
	 */
