...
public class LocalizationUtilTest {
  ...
  public void testImportExportOfMultibyteChars() throws Exception {
    ...
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_MODEL_1].[Category-TEST_WITH_日本語_CHARS].[name]"),"日本語");
  ...
  public void testNonHappyPaths() throws Exception {
    ...
    props.setProperty("[PhysicalModel-SampleDataDatasource].[PT_ORDERDETAILS2].[ORDERLINENUMBER].[name]","ORDERLINENUMBER");
  ...
  public void testSimpleModel() throws Exception {
    ...
    Assert.assertEquals(props.getProperty("[IPhysicalModel-SampleDataDatasource].[PT_ORDERDETAILS].[name]"),"ORDERDETAILS");
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_MODEL_1].[LogicalTable-BT_ORDERDETAILS_ORDERDETAILS].[name]"),"ORDERDETAILS");
    Assert.assertEquals(props.getProperty("[IPhysicalModel-SampleDataDatasource].[PT_ORDERS].[ORDERNUMBER].[name]"),"ORDERNUMBER");
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_MODEL_1].[Category-CAT_Category].[name]"),"Category");
    Assert.assertEquals(props.getProperty("[IPhysicalModel-SampleDataDatasource].[PT_ORDERDETAILS].[ORDERLINENUMBER].[name]"),"ORDERLINENUMBER");
    Assert.assertEquals(props.getProperty("[Base].[comments]"),"Comment String");
    Assert.assertEquals(props.getProperty("[IPhysicalModel-SampleDataDatasource].[PT_ORDERS].[STATUS].[name]"),"STATUS");
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_MODEL_1].[LogicalTable-BT_ORDERS_ORDERS].[name]"),"ORDERS");
    Assert.assertEquals(props.getProperty("[IPhysicalModel-SampleDataDatasource].[PT_ORDERDETAILS].[ORDERNUMBER].[name]"),"ORDERNUMBER");
  ...
  public void testLocalization() throws Exception {
    ...
    // category name
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_ORDERS].[Category-BC_CUSTOMER_W_TER_].[name]"), "Customer");
    // model name
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_ORDERS].[name]"), "Orders");
    // logical table name
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_ORDERS].[LogicalTable-BT_PRODUCTS_PRODUCTS].[name]"), "Products");
    // logical column comments
    Assert.assertEquals(props.getProperty("[LogicalModel-BV_ORDERS].[LogicalTable-BT_ORDERDETAILS_ORDERDETAILS].[BC_ORDERDETAILS_TOTAL].[comments]"), "This field is computed as Quantity Ordered times Price Sold");
